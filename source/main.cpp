...
class HexagonalAsteroid : public Asteroid {
public:
	HexagonalAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 10; }
	std::vector<std::unique_ptr<Asteroid>> OnDestroy(int screenW, int screenH) override {
		...
		int count = 3 + GetRandomValue(0, 3);
...
// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, SHRAPNEL = 6, RANDOM = 0 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::SHRAPNEL:
		return std::make_unique<HexagonalAsteroid>(w, h);
	default: {
...
enum class WeaponType { LASER, BULLET, SPREAD, SNAKE, COUNT };
class Projectile {
public:
	...
	bool Update(float dt) {
		if (type == WeaponType::SNAKE) {
			sineTime += dt/2;

			transform.position.y += physics.velocity.y * dt/2;

			float amplitude = 2500.f;
			float frequency = 25.f;
			transform.position.x += amplitude * cosf(frequency * sineTime) * dt/2;
		}
		else { transform.position = Vector2Add(transform.position, Vector2Scale(physics.velocity, dt)); }
...
inline static std::vector<Projectile> MakeProjectiles(WeaponType wt,
	const Vector2 pos, float speed)
{
	...
	if (wt == WeaponType::LASER) {
		...
		float Offset = 20.0f * DEG2RAD;
		Vector2 left = Vector2Rotate(vel, -Offset);
		Vector2 right = Vector2Rotate(vel, Offset);

		result.emplace_back(pos, left, 8, wt);
		result.emplace_back(pos, right, 8, wt);
	}
	else if (wt == WeaponType::SNAKE) {
		result.emplace_back(pos, vel, 10, wt);
	}
...
public:
	...
	void Update(float dt) override {
		if (alive) {
			if (IsKeyDown(KEY_A)) transform.rotation -= 180.0f * dt;
			if (IsKeyDown(KEY_D)) transform.rotation += 180.0f * dt;
			Vector2 forward = {
				cosf(transform.rotation * DEG2RAD),
				sinf(transform.rotation * DEG2RAD)
			};
			if (IsKeyDown(KEY_W)) transform.position = Vector2Add(transform.position, Vector2Scale(forward, speed * dt));
			if (IsKeyDown(KEY_S)) transform.position = Vector2Subtract(transform.position, Vector2Scale(forward, speed * dt));
		}
		else {
...
public:
	...
	void Run() {
		...
		WeaponType currentWeapon = WeaponType::SNAKE;
		float shotTimer = 0.f;
		...
		while (!WindowShouldClose()) {
			...
			if (IsKeyPressed(KEY_FOUR)) {
				currentShape = AsteroidShape::SHRAPNEL;
			}
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						std::vector<Projectile> newShots = MakeProjectiles(currentWeapon, p, projSpeed);
			...
			// Render everything
			{
				...
				if (turboMode) {
					DrawText("TURBO MODE", 10, 70, 20, ORANGE);
				}
				DrawText(TextFormat("Score: %d", score), Renderer::Instance().Width() - MeasureText(TextFormat("Score: %d", score), 30) - 10, 10, 30, YELLOW);
